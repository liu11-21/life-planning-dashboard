...
function normalizeFinanceStages(list) {
  return list.map((stage) => ({
    ...
    sideIncome: Number(stage.sideIncome) || 0,
...
function projectPlan() {
  ...
  for (let age = input.currentAge; age <= 100; age += 1) {
    ...
    rows.push({
      ...
      date: financial.date,
      income,
      mainIncome: financial.mainIncome,
      sideIncome: financial.sideIncome,
      expense,
      tax: financial.tax,
...
function bindEvents() {
  ...
  document.getElementById("financeStageRows").addEventListener("input", (event) => {
    ...
    const numericFields = ["annualIncome", "sideIncome", "incomeGrowth", "sideIncomeGrowth"];
